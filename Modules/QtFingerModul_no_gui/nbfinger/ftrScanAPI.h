...
#if defined(_WIN32)
#  include <windows.h>
  //необходимо сбрасывать код последней ошибки перед вызовом функций ftr
  inline int  ftrGetError(){ return (int)GetLastError(); }
  inline void ftrSetError(int e){ SetLastError((DWORD)e);}
#else
#  define WINAPI
#  define BOOL int
...
#  define PBOOL int*
#  include <errno.h>
  //коды ошибок FTR недействительны, но устанавливается в не-ноль (при отключении = 6)
  inline int  ftrGetError(){ return errno; }
  inline void ftrSetError(int e){ errno = e;}
#endif
