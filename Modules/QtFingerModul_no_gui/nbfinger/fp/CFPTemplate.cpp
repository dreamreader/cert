	...
	for(int j=0; j<tLen; j++){
		...
		for(int jj=0; jj<tLen; jj++)
		{
			//вычисление растояния между минуциями
                        r=(float)(minutiae2[ii].x-minutiae3[jj].x)*(minutiae2[ii].x-minutiae3[jj].x)+\
			...
			//Совпала минуция?
                        if((r<49)&&((gg<F_PI/8)||(gg>(2*F_PI-F_PI/8))))
			{
				m++;
				minutiae3[jj].x=9000;
				minutiae3[jj].y=9000;
...
FPShift* CFPTemplate::MatchingSpeedTriangle(FPPoint *tMinutiae, int tLen, FPPoint *sMinutiae, int sLen, int *numShift)
{
	...
	int *cmpTable=NULL, *cmp2=NULL, *Cmp=NULL, *CMP=NULL;	// вектор сопоставления минуций
	int Sum1, Sum2;
	...
	for(jj=0; jj<tLen; jj++){
		...
		if(cmp2 !=NULL) {delete [] cmp2; cmp2=NULL;}
