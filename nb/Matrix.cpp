...
namespace Nb {
  ...
  bool Matrix::Row::resize(int32_t ncols){
    if (isLocked() || !_m) { qDebug()<<_irow<<" "<<irow(); return false; }
  ...
  bool Matrix::Row::convert(const Matrix::Row &row){
    ...
    if ((ncols != (uint32_t)row.count()) || (ecols != row.meta().count())) return false;
  ...
  const nbMatrix Matrix::Row::toM() const{
    if (!_m) {  //Создание описания пустой матрицы
      ...
      nbMatrix m = {1,(uint32_t)count(),(nbMeta*)_m->meta()+irow(), (nbVbp**)_m->data()+irow()};
      return m;
    }
  }
