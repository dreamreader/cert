...
bool Provider::detachWindow()
{
    ...
    switch (errc = _iface->connectToWindow(-1, NULL))
    {
    case nbS_OK:
        break;
    default:
...
bool Provider::clearWindow(){
    ...
    switch (errc = _iface->resetBim())
    {
    case nbS_OK:
      break;
    default:;
      qDebug() << "resetBim failed, errc=" << Result (errc);
    }
...
bool Provider::resetInput()
{
    ...
    switch (errc = _iface->waitForBimReady(&null, PROVIDER_TIMEOUT)) {
      case nbS_OK:
        break;
